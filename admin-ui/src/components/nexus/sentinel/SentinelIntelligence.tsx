...
export default function SentinelIntelligence() {
  ...
  const fetchAll = async () => {
    try {
      const [aRes, pRes, rRes] = await Promise.allSettled([
        nexusApi.get<{ data?: Anomaly[] | { anomalies?: Anomaly[] } }>('/admin/sentinel-intelligence/anomalies'),
        nexusApi.get<{ data?: Prediction[] | { predictions?: Prediction[] } }>('/admin/sentinel-intelligence/predictions'),
        nexusApi.get<{ data?: Recommendation[] | { recommendations?: Recommendation[] } }>('/admin/sentinel-intelligence/recommendations'),
      ]);

      if (aRes.status === 'fulfilled') {
        const json = aRes.value;
        const d = json.data ?? json;
        setAnomalies(Array.isArray(d) ? d : (d as { anomalies?: Anomaly[] }).anomalies || []);
      }
      if (pRes.status === 'fulfilled') {
        const json = pRes.value;
        const d = json.data ?? json;
        setPredictions(Array.isArray(d) ? d : (d as { predictions?: Prediction[] }).predictions || []);
      }
      if (rRes.status === 'fulfilled') {
        const json = rRes.value;
        const d = json.data ?? json;
        setRecommendations(Array.isArray(d) ? d : (d as { recommendations?: Recommendation[] }).recommendations || []);
      }
    } catch {} finally {
      setLoading(false);
    }
  ...
  const runDetection = async () => {
    ...
    try {
      await nexusApi.post('/admin/sentinel-intelligence/anomalies/detect');
      await fetchAll();
  ...
  const runPredictions = async () => {
    ...
    try {
      await nexusApi.post('/admin/sentinel-intelligence/predictions/generate');
      await fetchAll();
    } catch {} finally { setActionLoading(null); }
  };
