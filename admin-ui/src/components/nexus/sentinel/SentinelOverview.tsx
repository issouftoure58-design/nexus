...
export default function SentinelOverview() {
  ...
  const [alerts, setAlerts] = useState<Array<{ id?: string; title?: string; message?: string; severity?: string; level?: string; metric?: string; type?: string }>>([]);
    ...
    const fetchAll = async () => {
      try {
        const [healthRes, statusRes, alertsRes] = await Promise.allSettled([
          nexusApi.get<{ data?: { healthScore?: HealthScore }; healthScore?: HealthScore }>('/admin/sentinel-intelligence/health-score'),
          nexusApi.get<{ services?: ServiceStatus[] | Record<string, ServiceStatus>; checks?: ServiceStatus[] | Record<string, ServiceStatus> }>('/nexus/sentinel/status'),
          nexusApi.get<{ data?: { alerts?: Array<Record<string, unknown>> } | Array<Record<string, unknown>> }>('/admin/sentinel-intelligence/alerts?status=active'),
        ]);

        if (healthRes.status === 'fulfilled') {
          const json = healthRes.value;
          const d = (json as { data?: { healthScore?: HealthScore } }).data || json;
          setHealth((d as { healthScore?: HealthScore }).healthScore || d as HealthScore);
        }
        ...
        if (statusRes.status === 'fulfilled') {
          ...
          if (Array.isArray(svc)) {
            ...
            setServices(Object.values(svc) as ServiceStatus[]);
          }
        }

        if (alertsRes.status === 'fulfilled') {
          const json = alertsRes.value;
          const d = (json as { data?: unknown }).data || json;
          setAlerts(Array.isArray(d) ? d : (d as { alerts?: Array<Record<string, unknown>> }).alerts || []);
        }
    ...
    <div className="flex items-center gap-3">
      ...
      <div className="flex-1 h-2.5 bg-slate-800 rounded-full overflow-hidden">
        <div
          ...
          style={{ width: `${Math.min(w, 100)}%`, transition: 'width 1s cubic-bezier(0.22, 1, 0.36, 1)' }}
        />
      </div>
...
function ServiceCard({ service, index }: { service: ServiceStatus; index: number }) {
  ...
  return (
    <div className={`flex items-center gap-2 px-3 py-2.5 rounded-lg bg-slate-800/40 hover:bg-slate-800/70 transition-all duration-500 ${visible ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-2'}`}>
...
function AlertRow({ alert, index }: { alert: { id?: string; title?: string; message?: string; severity?: string; level?: string; metric?: string; type?: string }; index: number }) {
  const [visible, setVisible] = useState(false);
