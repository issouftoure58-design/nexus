    ...
    const fetchAll = async () => {
      try {
        const [logsRes, statsRes] = await Promise.allSettled([
          nexusApi.get<{ data?: SecurityEvent[] | { events?: SecurityEvent[]; logs?: SecurityEvent[] } }>('/nexus/sentinel/security/logs'),
          nexusApi.get<{ data?: Record<string, unknown>; security?: Record<string, unknown>; stats?: Record<string, unknown>; rateLimit?: Record<string, unknown> }>('/nexus/sentinel/security/stats'),
        ]);

        if (logsRes.status === 'fulfilled') {
          const json = logsRes.value;
          const d = json.data ?? json;
          if (Array.isArray(d)) setLogs(d);
          else setLogs((d as { events?: SecurityEvent[]; logs?: SecurityEvent[] }).events || (d as { logs?: SecurityEvent[] }).logs || []);
        }
        if (statsRes.status === 'fulfilled') {
          const json = statsRes.value;
          const d = json.data ?? json;
          const sec = (d as Record<string, unknown>).security || (d as Record<string, unknown>).stats || d;
          const rl = (d as Record<string, unknown>).rateLimit || {} as Record<string, unknown>;
          setStats({
            total: (sec as Record<string, number>).total ?? 0,
            bySeverity: (sec as Record<string, Record<string, number>>).bySeverity ?? {},
            byType: (sec as Record<string, Record<string, number>>).byType ?? {},
            blockedIps: (rl as Record<string, number>).blocked ?? (sec as Record<string, number>).blockedIps ?? 0,
            rateLimited: (rl as Record<string, number>).totalTracked ?? (sec as Record<string, number>).rateLimited ?? 0,
          });
        }
