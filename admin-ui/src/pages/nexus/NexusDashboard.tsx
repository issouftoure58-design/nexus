          ...
          <>
            {data.env === 'development' && (
              <div className="mb-3 px-3 py-2 bg-yellow-950/30 rounded-lg border border-yellow-800/50 text-[11px] text-yellow-500/80 flex items-center gap-2">
                ...
                <span>Environnement local — Le serveur tourne sur votre ordinateur. En production, il tournera sur un serveur cloud dedie.</span>
              </div>
            )}

            {data.periodLabel && (
              <div className="mb-3 text-[11px] text-slate-500">
                Donnees cumulees <span className="text-slate-400 font-medium">{data.periodLabel}</span> — sauvegardees en base de donnees.
              </div>
            )}
            ...
            {healthScore != null && (
              <div className="mb-4 flex items-center gap-4 bg-slate-900 rounded-lg border border-slate-800 p-3">
                ...
                <div>
                  ...
                  <div className="text-[10px] text-slate-600">Score composite base sur les revenus, reservations, couts et disponibilite.</div>
                </div>
              </div>
            )}
  ...
  return (
    <div className={`p-4 rounded-lg border ${colors[color]} transition-all duration-300 hover:brightness-110`}>
      ...
      <div className={`text-2xl font-bold ${textColors[color]} tabular-nums`}>
        {decimals > 0 ? displayed.toFixed(decimals) : Math.round(displayed)}{suffix}
      </div>
    </div>
  );
}
