...
export default function NexusSettings() {
  ...
  useEffect(() => {
    const fetchStatus = async () => {
      try {
        const json = await nexusApi.get<SystemStatus>('/nexus/sentinel/status');
        setSystem(json);
      ...
      <div className="max-w-6xl mx-auto">
        ...
        <div className="bg-slate-900 rounded-lg border border-slate-800 mb-6 p-4">
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
            ...
            />
            <LiveInfoItem label="Uptime" value={formatUptime(uptime)} live />
            <LiveInfoItem
              label="Memoire Heap"
            ...
            />
            <LiveInfoItem label="Tenants actifs" value={String(system?.tenants ?? '---')} />
          </div>
        </div>
