...
// Find the latest pro tenant
const { data: tenants } = await supabase.from('tenants')
  .select('id, plan').order('created_at', { ascending: false }).limit(5);
const proTenant = tenants.find(t => t.plan === 'pro');
const tid = process.argv[2] || proTenant?.id;
if (!tid) { console.error('No Pro tenant found'); process.exit(1); }
...
const expected = getFeaturesForPlan('pro');
...
for (const m of shouldBeTrue) {
  check(`${m} = true (inclus Pro)`, actual[m] === true);
}
for (const m of shouldBeFalse) {
  check(`${m} = false (bloque Pro)`, !actual[m]);
}
...
check('users_max = 20', lim.users_max === 20);

// RESUME
console.log(`\n${'='.repeat(40)}`);
console.log(`RESULTAT: ${ok} OK, ${fail} FAIL`);
if (fail === 0) console.log('Tous les quotas Pro sont en place.');
